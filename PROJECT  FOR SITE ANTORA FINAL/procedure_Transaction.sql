create or replace procedure transac_account_number(ttaccNum in number)
	...
	cursor hello1 is
	select *from transaction@site_linkTahmeed where Account_number=ttaccNum;
	...
	cursor hello1 is
	select *from transaction@site_linkTahmeed where deposit>=ttdeposit;
	...
	cursor hello1 is
	select *from transaction@site_linkTahmeed where eid=tteid;
	...
	cursor hello1 is
	select *from transaction@site_linkTahmeed where tid=ttid;
	...
	cursor hello1 is
	select *from transaction@site_linkTahmeed where withdraw>=ttwith;
	...
	is
		...
		cursor mycursor is
		select *from transaction@site_linkTahmeed where transaction_date like '%'||tdate||'%';
	...
	create or replace procedure select_all_transaction
		...
		cursor mycursor is
		select *from transaction@site_linkTahmeed;
	...
	insert into transaction@site_linkTahmeed values(tttid,tdeposit,twithdraw,ttransaction_date,tEID,tAccount_number);
